...
export function DeveloperDetails({ pattern }: { pattern: ValenciaPattern }) {
  const [open, setOpen] = useState(false)
  return (
    <div className={`v-dev ${open ? 'v-dev--open' : ''}`}>
      <button className="v-dev__toggle" onClick={() => setOpen(v => !v)} aria-expanded={open}>
        Developer details {open ? '−' : '+'}
      </button>
      {open && (
        <div className="v-dev__body">
          <dl className="v-dev__list">
