...
export function MaskReveal({
  ...
  const inner = when === 'mount'
    ? <motion.div className="t-mask__item" initial={initial} animate={visible} transition={transition}>{children}</motion.div>
    : <motion.div className="t-mask__item" initial={initial} whileInView={visible} viewport={{ once: true, amount: 0.25 }} transition={transition}>{children}</motion.div>

  return <div className={`t-mask ${className ?? ''}`}>{inner}</div>
}
