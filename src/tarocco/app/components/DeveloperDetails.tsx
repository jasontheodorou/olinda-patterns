...
export function DeveloperDetails({ pattern }: { pattern: TaroccoPattern }) {
  const [open, setOpen] = useState(false)
  return (
    <div className={`t-dev ${open ? 'v-dev--open' : ''}`}>
      <button className="t-dev__toggle" onClick={() => setOpen(v => !v)} aria-expanded={open}>
        Developer details {open ? '−' : '+'}
      </button>
      {open && (
        <div className="t-dev__body">
          <dl className="t-dev__list">
