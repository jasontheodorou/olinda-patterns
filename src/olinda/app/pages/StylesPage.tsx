...
import { motionStyles, type OlindaStyle } from '../../styles/motionStyles'
import './StylesPage.css'

const STYLES: { id: OlindaStyle; name: string; description: string }[] = [
...
export function StylesPage() {
  ...
  if (active) {
    return (
      <div className="o-styles o-styles--detail">
        <div className="o-styles__crumb">
          <Link to="/styles">Styles</Link> · {active.name}
        </div>
        <h1 className="o-styles__title">{active.name}</h1>
        <p className="o-styles__lede">{active.description}</p>

        <section className="o-styles__preview">
          <motion.h2
            key={active.id}
            ...
            className="o-styles__preview-headline"
          >
            A large heading, in this style.
          </motion.h2>
        </section>

        <dl className="o-styles__tokens">
  ...
  return (
    <div className="o-styles">
      <header className="o-styles__head">
        <h1 className="o-styles__title">Styles</h1>
        <p className="o-styles__lede">
          Five semantic motion styles. Pick a mood; the numbers behind it are the same everywhere.
        </p>
      </header>

      <div className="o-styles__grid">
        {STYLES.map(s => (
          <Link key={s.id} to={`/styles/${s.id}`} className="o-styles__card">
            <div className="o-styles__card-name">{s.name}</div>
            <div className="o-styles__card-desc">{s.description}</div>
            <div className="o-styles__card-nums">
