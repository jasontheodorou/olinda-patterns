...
export function CollectionPage() {
  ...
  if (!collection) {
    return (
      <div className="o-coll">
        <p className="o-coll__missing">
  ...
  return (
    <div className="o-coll">
      <header className="o-coll__head">
        <div className="o-coll__crumb">
          <Link to="/examples">Examples</Link> · Collections
        </div>
        <h1 className="o-coll__title">{collection}</h1>
        <p className="o-coll__lede">{items.length} pattern{items.length === 1 ? '' : 's'} in this collection.</p>
      </header>

      <div className="o-coll__grid">
        {items.map(p => <PatternCard key={p.id} pattern={p} />)}
      </div>

      <nav className="o-coll__sibs" aria-label="Other collections">
        {COLLECTIONS.filter(c => c !== collection).map(c => (
          <Link key={c} to={`/collections/${c.toLowerCase()}`} className="o-coll__sib">{c}</Link>
        ))}
      </nav>
    </div>
  )
