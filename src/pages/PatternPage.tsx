      ...
      <Stack gap={0} maw={780}>
        ...
        <Box style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flexWrap: 'wrap', marginBottom: 32 }}>
          ...
          <Text style={{
            ...
            fontFamily: f, padding: '4px 10px', marginTop: 7, flexShrink: 0,
            background: pattern.status === 'ready' ? C.teal : 'transparent',
            color: pattern.status === 'ready' ? C.surface : C.muted,
            border: `1px solid ${pattern.status === 'ready' ? C.teal : C.border}`,
          }}>
            {pattern.status}
          </Text>
        </Box>
