# Olinda — architecture and integration guide for coding agents

This document tells any coding agent (Claude Code or otherwise) how to work with the Olinda pattern library. Read it before adding or modifying patterns.

## What Olinda is

A React + TypeScript + Vite + Motion for React pattern library. It ships:

- Eight **core motion behaviours** in `src/olinda/core/` (Reveal, MaskReveal, Stagger, useOlindaScroll, StickyScene, Parallax, LayoutTransition, PointerResponse).
- Ten **Ready patterns** in `src/olinda/patterns/` composed from the core.
- Five **semantic motion styles** in `src/olinda/styles/` (Quiet · Clear · Editorial · Bold · Playful).
- An **abstract theme contract** in `src/olinda/theme/` — every pattern reads `--o-*` CSS variables.
- A **single registry** in `src/olinda/registry/patterns.ts` from which the site, collections, prompts, and manifest are derived.
- A **generated manifest** at `src/olinda/registry/olinda.manifest.json` (do not hand-edit).
...
├── registry/            types.ts, patterns.ts, collections.ts, olinda.manifest.json
├── styles/              motionStyles.ts, energy.ts, speed.ts, resolveMotion.ts
└── theme/               contract.ts, defaultTheme.ts, OlindaThemeProvider.tsx
```
...
3. **Shuffle uses Fisher-Yates** with an injectable RNG — `fisherYatesShuffle` in `src/olinda/accessibility/reducedMotion.ts`. Never `sort(() => Math.random() - 0.5)`.
...
5. **`useOlindaScroll` returns `MotionValue`**, not React state. Never set React state on every scroll frame.
6. **StickyScene is CSS `position: sticky` first**; JS only for derived progress.
7. **Energy and speed are separate multipliers**. The single `resolveMotion(style, energy, speed)` helper (`src/olinda/styles/resolveMotion.ts`) is the sole consumer.
8. **Prompts are template strings** on the metadata (`prompt` field). `renderPrompt(pattern, controls)` in `src/olinda/agent/prompts.ts` does interpolation. Never inline `{{...}}` per pattern.
9. **Manifest is generated** by `scripts/build-manifest.ts` (runs as `prebuild`). Never hand-edit `olinda.manifest.json`.
...
Every pattern's `metadata.ts` exports a `OlindaPattern` conforming to `src/olinda/registry/types.ts`. Fields:
...
1. Create the folder `src/olinda/patterns/<slug>/` with the 5 files.
2. Consume `motionStyles`, `energy`, `speed`, and `resolveMotion` from `src/olinda/styles/`.
3. Read colours and type from `--o-*` CSS variables; never hardcode Transform brand values.
4. Support `prefers-reduced-motion` — content must be visible without motion.
5. Register in `src/olinda/registry/patterns.ts`.
6. Run `npm run build` — the `prebuild` script regenerates the manifest.

## Theme contract

`OlindaTheme` in `src/olinda/theme/contract.ts` is the abstract shape:
...
`OlindaThemeProvider` emits `--o-bg`, `--o-surface`, `--o-text`, `--o-muted`, `--o-primary`, `--o-accent`, `--o-font-display`, `--o-font-body`, `--o-font-mono`, `--o-radius-sm|md|lg`, `--o-space-page`, `--o-space-section` on a wrapper `div`. Patterns consume these via CSS. Any hardcoded colour in a pattern file is a bug.
...
1. Designer picks a pattern in the Olinda site.
...
5. Claude:
   ...
   - maps `--o-*` variables to whatever theme the target already has, or installs `OlindaThemeProvider`;
   - runs the target repo's build.

`npx olinda add <id>` and package-import modes are V3, not V2. Do not implement them.
...
- `npm run build` — must succeed. The `prebuild` script regenerates `olinda.manifest.json`.
