# Tarocco — architecture and integration guide for coding agents

This document tells any coding agent (Claude Code or otherwise) how to work with the Tarocco pattern library. Read it before adding or modifying patterns.

## What Tarocco is

A React + TypeScript + Vite + Motion for React pattern library. It ships:

- Eight **core motion behaviours** in `src/tarocco/core/` (Reveal, MaskReveal, Stagger, useTaroccoScroll, StickyScene, Parallax, LayoutTransition, PointerResponse).
- Ten **Ready patterns** in `src/tarocco/patterns/` composed from the core.
- Five **semantic motion styles** in `src/tarocco/styles/` (Quiet · Clear · Editorial · Bold · Playful).
- An **abstract theme contract** in `src/tarocco/theme/` — every pattern reads `--t-*` CSS variables.
- A **single registry** in `src/tarocco/registry/patterns.ts` from which the site, collections, prompts, and manifest are derived.
- A **generated manifest** at `src/tarocco/registry/tarocco.manifest.json` (do not hand-edit).
...
├── registry/            types.ts, patterns.ts, collections.ts, tarocco.manifest.json
├── styles/              motionStyles.ts, energy.ts, speed.ts, resolveMotion.ts
└── theme/               contract.ts, defaultTheme.ts, TaroccoThemeProvider.tsx
```
...
3. **Shuffle uses Fisher-Yates** with an injectable RNG — `fisherYatesShuffle` in `src/tarocco/accessibility/reducedMotion.ts`. Never `sort(() => Math.random() - 0.5)`.
...
5. **`useTaroccoScroll` returns `MotionValue`**, not React state. Never set React state on every scroll frame.
6. **StickyScene is CSS `position: sticky` first**; JS only for derived progress.
7. **Energy and speed are separate multipliers**. The single `resolveMotion(style, energy, speed)` helper (`src/tarocco/styles/resolveMotion.ts`) is the sole consumer.
8. **Prompts are template strings** on the metadata (`prompt` field). `renderPrompt(pattern, controls)` in `src/tarocco/agent/prompts.ts` does interpolation. Never inline `{{...}}` per pattern.
9. **Manifest is generated** by `scripts/build-manifest.ts` (runs as `prebuild`). Never hand-edit `tarocco.manifest.json`.
...
Every pattern's `metadata.ts` exports a `TaroccoPattern` conforming to `src/tarocco/registry/types.ts`. Fields:
...
1. Create the folder `src/tarocco/patterns/<slug>/` with the 5 files.
2. Consume `motionStyles`, `energy`, `speed`, and `resolveMotion` from `src/tarocco/styles/`.
3. Read colours and type from `--t-*` CSS variables; never hardcode Transform brand values.
4. Support `prefers-reduced-motion` — content must be visible without motion.
5. Register in `src/tarocco/registry/patterns.ts`.
6. Run `npm run build` — the `prebuild` script regenerates the manifest.

## Theme contract

`TaroccoTheme` in `src/tarocco/theme/contract.ts` is the abstract shape:
...
`TaroccoThemeProvider` emits `--t-bg`, `--t-surface`, `--t-text`, `--t-muted`, `--t-primary`, `--t-accent`, `--t-font-display`, `--t-font-body`, `--t-font-mono`, `--t-radius-sm|md|lg`, `--t-space-page`, `--t-space-section` on a wrapper `div`. Patterns consume these via CSS. Any hardcoded colour in a pattern file is a bug.

## Reduced motion + accessibility

`src/tarocco/accessibility/reducedMotion.ts`:
...
1. Designer picks a pattern in the Tarocco site.
...
5. Claude:
   ...
   - maps `--t-*` variables to whatever theme the target already has, or installs `TaroccoThemeProvider`;
   - runs the target repo's build.

`npx tarocco add <id>` and package-import modes are V3, not V2. Do not implement them.
...
- `npm run build` — must succeed. The `prebuild` script regenerates `tarocco.manifest.json`.
