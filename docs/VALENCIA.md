# Valencia — architecture and integration guide for coding agents

This document tells any coding agent (Claude Code or otherwise) how to work with the Valencia pattern library. Read it before adding or modifying patterns.

## What Valencia is

A React + TypeScript + Vite + Motion for React pattern library. It ships:

- Eight **core motion behaviours** in `src/valencia/core/` (Reveal, MaskReveal, Stagger, useValenciaScroll, StickyScene, Parallax, LayoutTransition, PointerResponse).
- Ten **Ready patterns** in `src/valencia/patterns/` composed from the core.
- Five **semantic motion styles** in `src/valencia/styles/` (Quiet · Clear · Editorial · Bold · Playful).
...
├── registry/            types.ts, patterns.ts, collections.ts, valencia.manifest.json
├── styles/              motionStyles.ts, energy.ts, speed.ts, resolveMotion.ts
└── theme/               contract.ts, defaultTheme.ts, ValenciaThemeProvider.tsx
```
...
5. **`useValenciaScroll` returns `MotionValue`**, not React state. Never set React state on every scroll frame.
6. **StickyScene is CSS `position: sticky` first**; JS only for derived progress.
7. **Energy and speed are separate multipliers**. The single `resolveMotion(style, energy, speed)` helper (`src/valencia/styles/resolveMotion.ts`) is the sole consumer.
8. **Prompts are template strings** on the metadata (`prompt` field). `renderPrompt(pattern, controls)` in `src/valencia/agent/prompts.ts` does interpolation. Never inline `{{...}}` per pattern.
9. **Manifest is generated** by `scripts/build-manifest.ts` (runs as `prebuild`). Never hand-edit `valencia.manifest.json`.
...
Every pattern's `metadata.ts` exports a `ValenciaPattern` conforming to `src/valencia/registry/types.ts`. Fields:
...
1. Create the folder `src/valencia/patterns/<slug>/` with the 5 files.
2. Consume `motionStyles`, `energy`, `speed`, and `resolveMotion` from `src/valencia/styles/`.
3. Read colours and type from `--v-*` CSS variables; never hardcode Transform brand values.
4. Support `prefers-reduced-motion` — content must be visible without motion.
5. Register in `src/valencia/registry/patterns.ts`.
6. Run `npm run build` — the `prebuild` script regenerates the manifest.

## Theme contract

`ValenciaTheme` in `src/valencia/theme/contract.ts` is the abstract shape:
...
`ValenciaThemeProvider` emits `--v-bg`, `--v-surface`, `--v-text`, `--v-muted`, `--v-primary`, `--v-accent`, `--v-font-display`, `--v-font-body`, `--v-font-mono`, `--v-radius-sm|md|lg`, `--v-space-page`, `--v-space-section` on a wrapper `div`. Patterns consume these via CSS. Any hardcoded colour in a pattern file is a bug.
...
1. Designer picks a pattern in the Valencia site.
...
5. Claude:
   ...
   - maps `--v-*` variables to whatever theme the target already has, or installs `ValenciaThemeProvider`;
   - runs the target repo's build.

`npx valencia add <id>` and package-import modes are V3, not V2. Do not implement them.
...
- `npm run build` — must succeed. The `prebuild` script regenerates `valencia.manifest.json`.
