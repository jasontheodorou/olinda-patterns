...
Read @docs/v2-build-pack/05_CLAUDE_CODE_BUILD_BRIEF.md for the V2 brief, @docs/v2-build-pack/07_OLINDA_PATTERN_SPEC.md for the per-pattern contract, and @docs/v2-build-pack/08_RELEASE_REVIEW.md for known gotchas.
...
- `useOlindaScroll` returns a `MotionValue`, not React state.
...
- The manifest at `src/olinda/registry/olinda.manifest.json` is generated by `scripts/build-manifest.ts` — never hand-edited.
- Per-pattern `prompt` is a template string in metadata; the `renderPrompt(pattern, controls)` helper does interpolation.
- Every Olinda pattern folder ships exactly 5 files: `<Internal>.tsx`, `<Internal>.demo.tsx`, `<Internal>.css`, `metadata.ts`, `index.ts`.
